...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    const banBtn = document.getElementById('ban') as HTMLButtonElement;
...
async function banPlayer() {
    ...
    if (player == "-1") {
        ...
        responseEl.innerHTML = `You must select a player to ban from the dropdown`;
        return;
    }
    ...
    const responseStatus = await SendApiRequest("ban", data, serverIp, serverPort, serverPassword);

    if (responseStatus == 200) {
        responseEl.classList = "";
        responseEl.classList.add('text-success');
        responseEl.innerHTML = `Successfully banned: ${player}`;
    } else {
        responseEl.classList = "";
        responseEl.classList.add('text-danger');
        responseEl.innerHTML = `There was a problem trying to ban: ${player}`;
    }
}
